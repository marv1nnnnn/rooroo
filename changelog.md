...
- Updated `customInstructions` for the `Workflow Coordinator` (to v3.13) and `Strategic Planner` (to v3.12) agents:
- **Workflow Coordinator (v3.13):**
    - **Enhanced Interactive Error Handling for Missing Agent State Files:**
        - When an agent's state file (`.state/tasks/{taskId}.json`) is missing, the Coordinator no longer just logs an error and stops.
        - It now logs a `missing_state_file_error` to `task_log.jsonl`.
        ...
        - Coordinator behavior is now conditional on user's choice (e.g., re-attempting file read, treating task as failed, delegating to Planner, or aborting).
    - **Enhanced Interactive Error Handling for Agent Task Failures:**
        - When an agent reports `status: "Failed"` or `status: "Error"` in its state file:
        - The Coordinator logs the `task_failed` event to `task_log.jsonl`.
        ...
        - Coordinator behavior is now conditional on user's choice (e.g., re-delegating the task, skipping, delegating failure analysis to Planner, or aborting).
    - **Tool Usage Update:** Explicitly added `ask_followup_question` to its described toolset.
    - **New `task_log.jsonl` Event Types:** Implicitly introduces new `event_type` values related to these interactive error handling flows, such as: `missing_state_file_error`, `user_request_recheck_state_file`, `user_mark_as_failed_due_to_missing_state_file`, `user_delegate_system_issue_to_planner`, `user_retry_request`, `user_skip_request`, `user_delegate_to_planner_request`, `user_abort_request`.
- **Strategic Planner (v3.12):**
    - **System Issue Analysis:** Explicitly instructed to analyze and provide recommendations when invoked by the Coordinator for system-level issues, such as an agent failing to produce a state file. This aligns with the Coordinator's new `PlannerInvestigateSystemIssue` option.
    - **Refined `suggested_mode` Guidance:** Provided more detailed examples for assigning `suggested_mode` based on task `type` during planning (e.g., `coder-monk` for `debug` and `code` tasks, `solution-architect` for `design`/`spec` tasks, etc.). This is a clarification and expansion of previous guidance.
